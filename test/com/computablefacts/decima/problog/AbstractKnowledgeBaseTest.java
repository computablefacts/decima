...
public class AbstractKnowledgeBaseTest {
  ...
  public void testFnAssertJson() {
    ...
    Assert.assertEquals(51,
        kb.nbFacts(new Literal("json_path", newVar(), newVar(), newVar(), newVar())));
    Assert.assertEquals(51, kb.nbFacts(new Literal("json_path",
        Lists.newArrayList(newVar(), newVar(), newVar(), newVar(), newVar()))));
  ...
  public void testFnAssertCsv() {
    ...
    Clause clause = parseClause(
        String.format("json_path(\"aIMuk3ze\", \"data\", \"3\", \"rawOutput\", \"%s\").", csv));
    Clause rule = parseClause(
  ...
  public void testMockMaterializeFactsQueryWithoutFixedTerms() {
    ...
    String rule1 =
        "clients(FirstName, LastName, Email) :- fn_mock_materialize_facts(\"http://localhost:3000/crm1\", \"first_name\", FirstName, \"last_name\", LastName, \"email\", Email).";
  ...
  public void testMockMaterializeFactsQueryWithCarriageReturnAndLineFeed() {
    ...
    String rule1 =
        "fichier_dab(PATH, TEXT) :- fn_mock_materialize_facts(\"https://localhost/facts/dab/fichier\", \"metadata.path\", _, PATH, \"content.text\", _, TEXT).";
    String rule2 =
        "fichier_vam(PATH, TEXT) :- fn_mock_materialize_facts(\"https://localhost/facts/vam/fichier\", \"metadata.path\", _, PATH, \"content.text\", _, TEXT).";
    String rule3 =
    ...
    Literal literal =
        new Literal("fichier_duplique", Lists.newArrayList(newConst("/var/sftp/file2.pdf"),
        ...
        if (parameters.get(0).asString().equals("https://localhost/facts/dab/fichier")) {
          if (("_".equals(path) || "/var/sftp/file1.pdf".equals(path))
            ...
            literals.add(new Literal("fn_mock_materialize_facts",
                Lists.newArrayList(newConst("https://localhost/facts/dab/fichier"),
                    newConst("metadata.path"), newConst("*"), newConst("/var/sftp/file1.pdf"),
                    newConst("content.text"), newConst("*"),
          ...
          if (("_".equals(path) || "/var/sftp/file2.pdf".equals(path)) && ("_".equals(text)
            ...
            literals.add(new Literal("fn_mock_materialize_facts",
                Lists.newArrayList(newConst("https://localhost/facts/dab/fichier"),
                    newConst("metadata.path"), newConst("*"), newConst("/var/sftp/file2.pdf"),
                    newConst("content.text"), newConst("*"),
                    newConst("The quick brown fox\njumps over\r\nthe lazy dog"))));
          }
        } else {
          if (("_".equals(path) || "/var/sftp/file2.pdf".equals(path)) && ("_".equals(text)
            ...
            Literal literal = new Literal("fn_mock_materialize_facts",
                Lists.newArrayList(newConst("https://localhost/facts/vam/fichier"),
                    newConst("metadata.path"), newConst("*"), newConst("/var/sftp/file2.pdf"),
                    newConst("content.text"), newConst("*"),
                    newConst("The quick brown fox\njumps over\r\nthe lazy dog")));
          ...
          if (("_".equals(path) || "/var/sftp/file3.pdf".equals(path))
            ...
            Literal literal = new Literal("fn_mock_materialize_facts",
                Lists.newArrayList(newConst("https://localhost/facts/vam/fichier"),
                    newConst("metadata.path"), newConst("*"), newConst("/var/sftp/file3.pdf"),
                    newConst("content.text"), newConst("*"),
                    newConst("The quick brown fox jumps over the lazy dog")));
            literals.add(literal);
          }
      ...
      public BoxedType<?> evaluate(List<BoxedType<?>> parameters) {
        ...
        literals.add(parseClause(
            "fn_mock_materialize_facts(\"https://localhost/facts/dab/fichier\", \"metadata.path\", \"*\", \"/var/sftp/file1.pdf\", \"content.text\", \"*\", \"lhs := rhs — function etc. definition\").")
                .head());
        literals.add(parseClause(
            "fn_mock_materialize_facts(\"https://localhost/facts/dab/fichier\", \"metadata.path\", \"*\", \"/var/sftp/file2.pdf\", \"content.text\", \"*\", \"x == val — test equality or represent a symbolic equation (!¤u003d for unequal)\").")
                .head());
        literals.add(parseClause(

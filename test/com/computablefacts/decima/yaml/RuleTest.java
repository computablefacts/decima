...
public class RuleTest {
  ...
  public void testToStringBodyWithOneLiteral() {
    ...
    Rule rule =
        new Rule("stress", "", 0.3, "X", Lists.newArrayList("person(X)").toArray(new String[1]));
  ...
  public void testToStringBodyWithMoreThanOneLiteral() {
    ...
    Rule rule = new Rule("smokes", "", 1.0, "X",
        Lists.newArrayList("stress(X)", "friend(X, Y), influences(Y, X), smokes(Y)")
            .toArray(new String[2]));

    Assert.assertEquals("1.0::smokes(X) :- stress(X).\n"
